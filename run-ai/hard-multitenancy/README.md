# NVIDIA Run:ai Hard Multitenancy Stack

Deploys a fully self-contained NVIDIA Run:ai environment using vCluster with **hard isolation**. Both the NVIDIA Run:ai control plane and each tenant agent run inside dedicated vClusters with private auto-provisioned nodes.
...
- **Dedicated control plane** — the NVIDIA Run:ai CP is deployed inside its own vCluster, fully managed by this stack.
...
| **NVIDIA Run:ai registry credentials** | Base64-encoded Docker config JSON for pulling NVIDIA Run:ai images from JFrog |
...
The stack will:
1. Create a CP vCluster with auto nodes and deploy the NVIDIA Run:ai control plane
2. Wait for the CP API to become healthy (up to 30 minutes, configurable)
3. Register each agent cluster via the NVIDIA Run:ai REST API
4. Create an agent vCluster per agent with auto nodes, GPU operator, and the NVIDIA Run:ai agent
...
Controls the VMs backing the NVIDIA Run:ai control-plane vCluster:
...
| runai\_cp\_domain | Domain name for the NVIDIA Run:ai control plane (e.g. runai.10.0.0.1.nip.io) | `string` | n/a | yes |
...
| cp\_health\_check\_retries | Number of retries when waiting for the NVIDIA Run:ai API to become healthy | `number` | `180` | no |
...
| runai\_admin\_email | Admin email for initial NVIDIA Run:ai login. Validated by the shared runai-auth module. | `string` | `"admin@run.ai"` | no |
| runai\_admin\_password | Admin password for initial NVIDIA Run:ai login. Auto-generated if not provided. | `string` | `null` | no |
...
| runai\_chart\_version | NVIDIA Run:ai Helm chart version (used for both CP and agent) | `string` | `"2.24.18"` | no |
| runai\_cp\_chart\_repo | Helm chart repository for the NVIDIA Run:ai control plane | `string` | `"https://runai.jfrog.io/artifactory/cp-charts-prod"` | no |
...
| control\_plane\_url | URL of the NVIDIA Run:ai control plane |
...
| runai\_admin\_password | NVIDIA Run:ai admin password (auto-generated if not provided) |
<!-- END_TF_DOCS -->
...
2. Check NVIDIA Run:ai pods inside: `kubectl --kubeconfig ./<name>-cp-kubeconfig.yaml get pods -n runai-backend`
...
2. Check NVIDIA Run:ai agent pods: `kubectl --kubeconfig ./<name>-<agent-name>-kubeconfig.yaml get pods -n runai`
...
|---|---|---|
| NVIDIA Run:ai control plane | Dedicated vCluster (self-contained) | External (shared, pre-existing) |
