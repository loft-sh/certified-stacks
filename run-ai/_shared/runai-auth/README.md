...
Authenticates with the Run:ai control plane and returns a bearer token. Used internally by `runai-project` and `runai-cluster-registration`.
...
| Name | Description |
|------|-------------|
| access\_token | Bearer token for Run:ai API requests |
<!-- END_TF_DOCS -->
